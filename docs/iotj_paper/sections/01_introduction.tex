...
Gateway-side physical-layer authentication can therefore complement protocol-layer security without requiring additional hardware on constrained LoRa nodes~\cite{lorawan_spec,hamdaoui2022deeplearning}.
...
Fig.~\ref{fig:application_scenario} illustrates the gateway-side authentication scenario considered in this paper.

\begin{figure*}[!t]
\centering
\begin{adjustbox}{max width=\textwidth,center}
\input{figures/fig0_application_scenario_tikz.tex}
\end{adjustbox}
\caption{Gateway-side LoRa device authentication scenario under deployment shifts. The gateway captures IQ windows and spectral/OOB views from enrolled or suspicious transmitters and performs file-level RF fingerprint authentication. Deployment factors such as day drift, distance, location, configuration, and receiver front-end mismatch can change the observed fingerprints.}
...
We propose \model, an OOB-guided cross-attentive RF-HSTU hybrid model for LoRa device authentication.
